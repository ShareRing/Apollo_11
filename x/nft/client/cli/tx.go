...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			denom := args[0]
			tokenID := args[1]
			digitalHash := viper.GetString(flagDigitalHash)

			msg := types.NewMsgEditNFTDigitalHash(cliCtx.GetFromAddress(), tokenID, denom, digitalHash)
			return utils.GenerateOrBroadcastMsgs(cliCtx, txBldr, []sdk.Msg{msg})
		},
	}
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgMintNFT(cliCtx.GetFromAddress(), recipient, tokenID, denom, tokenURI, digitalHash)
			return utils.GenerateOrBroadcastMsgs(cliCtx, txBldr, []sdk.Msg{msg})
		},
	}

	cmd.Flags().String(flagTokenURI, "", "URI for supplemental off-chain metadata (should return a JSON object)")
	cmd.Flags().String(flagDigitalHash, "", "DigitalHash for supplemental off-chain metadata (should return a JSON object)")
