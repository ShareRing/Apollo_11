...
func TestInitGenesis(t *testing.T) {
	...
	nft1 := nft.NewBaseNFT(id, address, tokenURI1,digitilaHash1)
	nft2 := nft.NewBaseNFT(id2, address, tokenURI1,digitilaHash1)
	nft3 := nft.NewBaseNFT(id3, address, tokenURI1,digitilaHash1)
	nfts := nft.NewNFTs(&nft1, &nft2, &nft3)
	collection := nft.NewCollection(denom, nfts)

	nftx := nft.NewBaseNFT(id, address2, tokenURI1,digitilaHash1)
	nft2x := nft.NewBaseNFT(id2, address2, tokenURI1,digitilaHash1)
	nft3x := nft.NewBaseNFT(id3, address2, tokenURI1,digitilaHash1)
