...
import (
	...
	"github.com/ShareRing/nft-module/x/nft"
...
func TestEditNFTDigitalHashMsg(t *testing.T) {
	app, ctx := createTestApp(false)
	h := nft.GenericHandler(app.NFTKeeper)
	...
	failingEditDigitalHash := types.NewMsgEditNFTDigitalHash(address, id, denom2, digitilaHash2)

	res, err := h(ctx, failingEditDigitalHash)
	require.Error(t, err)

	// Define MsgEditNFTDigitalHash
	editNFTDigitalHash := types.NewMsgEditNFTDigitalHash(address, id, denom, digitilaHash2)

	res, err = h(ctx, editNFTDigitalHash)
	require.NoError(t, err)
	...
	for _, event := range res.Events {
		for _, attribute := range event.Attributes {
			...
			case sender:
				require.Equal(t, value, address.String())
			case digitalHash:
				require.Equal(t, value, digitilaHash2)
			default:
				require.Fail(t, fmt.Sprintf("unrecognized event %s", key))
			}
		}
	...
	nftAfterwards, err := app.NFTKeeper.GetNFT(ctx, denom, id)
	require.NoError(t, err)
	require.Equal(t, digitilaHash2, nftAfterwards.GetDigitalHash())
}
...
func TestMintNFTMsg(t *testing.T) {
	...
	// Define MsgMintNFT
	mintNFT := types.NewMsgMintNFT(address, address, id, denom, tokenURI, digitalHash)
	...
	for _, event := range res.Events {
		...
		for _, attribute := range event.Attributes {
			...
			case digitalHash:
				require.Equal(t, value, digitalHash)
			default:
				require.Fail(t, fmt.Sprintf("unrecognized event %s", key))
			}
		}
