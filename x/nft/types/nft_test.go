...
func TestBaseNFTGetMethods(t *testing.T) {
	...
	require.Equal(t, digitalHash1, testNFT.GetDigitalHash())
}
...
func TestBaseNFTSetMethods(t *testing.T) {
	...
	testNFT.EditDigitalHash(digitalHash2)
	require.Equal(t, digitalHash2, testNFT.GetDigitalHash())
}
...
TokenURI:		%s
DigitalHash:	%s`,
		id, address, tokenURI, digitalHash1)
...
func TestNFTsAppendMethod(t *testing.T) {
	...
	testNFT4 := NewBaseNFT(id4, address, tokenURI, digitalHash1)
	testNFT5 := NewBaseNFT(id5, address, tokenURI, digitalHash1)
...
Owner:			%s
TokenURI:		%s
DigitalHash:	%s`, id, address, tokenURI, digitalHash1))
}
...
func TestNFTsMarshalUnmarshalJSON(t *testing.T) {
	...
	require.Equal(t, string(bz),
		fmt.Sprintf(`{"%s":{"id":"%s","owner":"%s","token_uri":"%s","digital_hash":"%s"}}`,
			id, id, address.String(), tokenURI, digitalHash1))
