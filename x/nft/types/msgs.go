...
/* --------------------------------------------------------------------------- */
// MsgEditNFTDigitalHash
/* --------------------------------------------------------------------------- */

// MsgEditNFTDigitalHash edits an NFT's digital hash
type MsgEditNFTDigitalHash struct {
	Sender      sdk.AccAddress `json:"sender" yaml:"sender"`
	ID          string         `json:"id" yaml:"id"`
	Denom       string         `json:"denom" yaml:"denom"`
	DigitalHash string         `json:"digital_hash" yaml:"digital_hash"`
}

// NewMsgEditNFTDigitalHash is a constructor function for MsgSetName
func NewMsgEditNFTDigitalHash(sender sdk.AccAddress, id,
	denom, digitalHash string,
) MsgEditNFTDigitalHash {
	return MsgEditNFTDigitalHash{
		Sender:      sender,
		ID:          strings.TrimSpace(id),
		Denom:       strings.TrimSpace(denom),
...
// Route Implements Msg
func (msg MsgEditNFTDigitalHash) Route() string { return RouterKey }

// Type Implements Msg
func (msg MsgEditNFTDigitalHash) Type() string { return "edit_nft_digital_hash" }

// ValidateBasic Implements Msg.
func (msg MsgEditNFTDigitalHash) ValidateBasic() error {
	if msg.Sender.Empty() {
		return sdkerrors.Wrap(sdkerrors.ErrInvalidAddress, "invalid sender address")
	}
	if strings.TrimSpace(msg.ID) == "" {
...
// GetSignBytes Implements Msg.
func (msg MsgEditNFTDigitalHash) GetSignBytes() []byte {
	bz := ModuleCdc.MustMarshalJSON(msg)
	return sdk.MustSortJSON(bz)
}

// GetSigners Implements Msg.
func (msg MsgEditNFTDigitalHash) GetSigners() []sdk.AccAddress {
	return []sdk.AccAddress{msg.Sender}
}
...
func NewMsgMintNFT(sender, recipient sdk.AccAddress, id, denom, tokenURI, digitalHash string) MsgMintNFT {
	return MsgMintNFT{
		Sender:      sender,
		Recipient:   recipient,
		ID:          strings.TrimSpace(id),
