...
func TestNewCollection(t *testing.T) {
	testNFT := NewBaseNFT(id, address, tokenURI, digitalHash1)
	testNFT2 := NewBaseNFT(id2, address, tokenURI, digitalHash1)
...
func TestCollectionUpdateNFTMethod(t *testing.T) {
	testNFT := NewBaseNFT(id, address, tokenURI, digitalHash1)
	testNFT2 := NewBaseNFT(id2, address2, tokenURI2, digitalHash2)
	testNFT3 := NewBaseNFT(id, address2, tokenURI2, digitalHash2)
	nfts := NewNFTs(&testNFT)
	collection := NewCollection(denom, nfts)
...
func TestCollectionDeleteNFTMethod(t *testing.T) {
	testNFT := NewBaseNFT(id, address, tokenURI, digitalHash1)
	testNFT2 := NewBaseNFT(id2, address2, tokenURI2, digitalHash2)
	testNFT3 := NewBaseNFT(id3, address, tokenURI, digitalHash1)
	nfts := NewNFTs(&testNFT, &testNFT2)
	collection := NewCollection(denom, nfts)
...
Owner:			%s
TokenURI:		%s
DigitalHash:	%s`, denom, id, address.String(), tokenURI, digitalHash1,
			id2, address2.String(), tokenURI2, digitalHash2))
}
...
func TestNewCollections(t *testing.T) {
	...
	testNFT := NewBaseNFT(id, address, tokenURI, digitalHash2)
	nfts := NewNFTs(&testNFT)
	collection := NewCollection(denom, nfts)
...
TokenURI:		%s
DigitalHash:	%s`, denom, id, address.String(), tokenURI, digitalHash1,
		denom2, id2, address2.String(), tokenURI2, digitalHash2), collections.String())
}
...
func TestCollectionMarshalAndUnmarshalJSON(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, string(bz), fmt.Sprintf(`{"%s":{"nfts":{"%s":{"id":"%s","owner":"%s","token_uri":"%s","digital_hash":"%s"}}},"%s":{"nfts":{"%s":{"id":"%s","owner":"%s","token_uri":"%s","digital_hash":"%s"}}}}`,
		denom, id, id, address.String(), tokenURI, digitalHash1,
		denom2, id2, id2, address2.String(), tokenURI2, digitalHash2,
	))
