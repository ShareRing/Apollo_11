...
func TestMsgMsgMintNFTValidateBasicMethod(t *testing.T) { //todo
	newMsgMintNFT := NewMsgMintNFT(nil, address2, id, denom, tokenURI, digitalHash1)
	err := newMsgMintNFT.ValidateBasic()
	require.Error(t, err)

	newMsgMintNFT = NewMsgMintNFT(address, nil, id, denom, tokenURI, digitalHash1)
	err = newMsgMintNFT.ValidateBasic()
	require.Error(t, err)

	newMsgMintNFT = NewMsgMintNFT(address, address2, "", denom, tokenURI, digitalHash1)
	err = newMsgMintNFT.ValidateBasic()
	require.Error(t, err)

	newMsgMintNFT = NewMsgMintNFT(address, address2, id, "", tokenURI, digitalHash1)
	err = newMsgMintNFT.ValidateBasic()
	require.Error(t, err)

	newMsgMintNFT = NewMsgMintNFT(address, address2, id, denom, tokenURI, digitalHash1)
	err = newMsgMintNFT.ValidateBasic()
	require.NoError(t, err)
}
...
func TestMsgMintNFTGetSignBytesMethod(t *testing.T) {
	...
	require.Equal(t, string(sortedBytes), fmt.Sprintf(`{"type":"cosmos-sdk/MsgMintNFT","value":{"denom":"%s","digital_hash":"%s","id":"%s","recipient":"%s","sender":"%s","token_uri":"%s"}}`,
		denom, digitalHash1, id, address2.String(), address.String(), tokenURI,
	))
}
