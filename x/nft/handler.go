...
func GenericHandler(k keeper.Keeper) sdk.Handler {
	return func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		...
		case types.MsgEditNFTDigitalHash:
			return HandleMsgEditNFTDigitalHash(ctx, msg, k)
...
func HandleMsgEditNFTMetadata(ctx sdk.Context, msg types.MsgEditNFTMetadata, k keeper.Keeper,
	...
	err = k.IsURIExists(ctx, msg.TokenURI)
	if err != nil {
		return nil, err
	}
...
// HandleMsgEditNFTDigitalHash handles MsgEditNFTDigitalHash
func HandleMsgEditNFTDigitalHash(ctx sdk.Context, msg types.MsgEditNFTDigitalHash, k keeper.Keeper,
) (*sdk.Result, error) {
	nft, err := k.GetNFT(ctx, msg.Denom, msg.ID)
	if err != nil {
		return nil, err
	...
	err = k.IsDigitalHashExists(ctx, msg.DigitalHash)
	if err != nil {
		return nil, err
	}

	//update digital hash of nft
	nft.EditDigitalHash(msg.DigitalHash)
	err = k.UpdateNFT(ctx, msg.Denom, nft)
	if err != nil {
		return nil, err
	}
	...
	ctx.EventManager().EmitEvents(sdk.Events{
		sdk.NewEvent(
			types.EventTypeEditNFTDigitalHash,
			sdk.NewAttribute(types.AttributeKeyDenom, msg.Denom),
			sdk.NewAttribute(types.AttributeKeyNFTID, msg.ID),
			sdk.NewAttribute(types.AttributeKeyNFTDigitalHash, msg.DigitalHash),
		),
		sdk.NewEvent(
			sdk.EventTypeMessage,
			sdk.NewAttribute(sdk.AttributeKeyModule, types.AttributeValueCategory),
...
func HandleMsgMintNFT(ctx sdk.Context, msg types.MsgMintNFT, k keeper.Keeper,
	...
	nft := types.NewBaseNFT(msg.ID, msg.Recipient, msg.TokenURI, msg.DigitalHash)
