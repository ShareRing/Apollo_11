...
func TestGetOwners(t *testing.T) {
	...
	nft2 := types.NewBaseNFT(id2, address2, tokenURI2, digitalHash2)
	err = app.NFTKeeper.MintNFT(ctx, denom, &nft2)
	require.NoError(t, err)

	nft3 := types.NewBaseNFT(id3, address3, tokenURI3, digitalHash3)
	...
	nft = types.NewBaseNFT(id, address, tokenURI4, digitalHash4)
	err = app.NFTKeeper.MintNFT(ctx, denom2, &nft)
	require.NoError(t, err)

	nft2 = types.NewBaseNFT(id2, address2, tokenURI5, digitalHash5)
	err = app.NFTKeeper.MintNFT(ctx, denom2, &nft2)
	require.NoError(t, err)

	nft3 = types.NewBaseNFT(id3, address3, tokenURI6, digitalHash6)
...
func TestSetOwners(t *testing.T) {
	...
	nft = types.NewBaseNFT(id2, address2, tokenURI2, digitalHash2)
	err = app.NFTKeeper.MintNFT(ctx, denom, &nft)
	require.NoError(t, err)
